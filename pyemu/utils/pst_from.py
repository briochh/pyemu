        ...
        if df_tpl.duplicated(["x", "y"]).any():
            ...
            if len(index_cols) > 2:
                ...
                if xy_in_idx is not None:
                    for idx in sorted(xy_in_idx, reverse=True): #
                        third_d.pop(idx)
                elif ij_in_idx is not None:
                    for idx in sorted(ij_in_idx, reverse=True):
                        third_d.pop(idx)
                ...
                pflogger.warn(
                    ...
                    "passed - using index_col[{0}] for third dimensions"
                    "-- This may break parameter correlations".format(third_d[-1])
                )
                for col in use_cols:
